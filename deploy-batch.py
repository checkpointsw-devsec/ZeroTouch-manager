...
CSV Formats:
    Smart-1 Cloud (Gaia gateways):
        ...
        SDWAN (optional: true/false — assign the gateway to an SD-WAN profile after deployment),
        SDWAN-Profile (optional: SD-WAN profile name, used when SDWAN is true)
...
Note:
    SD-WAN profile assignment (SDWAN / SDWAN-Profile columns) is only supported for the
    Smart-1 Cloud (s1c) and MDS/SMS (sms) deployment types. It is ignored for lsm and smp.
"""
...
class BatchDeployer:
    ...
    def assign_sdwan_profile(self, gateway_name: str, profile_name: str) -> bool:
        """
        Assign a deployed gateway to an SD-WAN profile via the Gateway Deployer API.

        Only supported for Smart-1 Cloud (s1c) and MDS/SMS (sms) deployments.
        Returns True on success, False otherwise. Failures are logged as warnings
        and do not fail the underlying gateway deployment.
        """
        self.log(f"  Assigning {gateway_name} to SD-WAN profile '{profile_name}'...")
        payload = {"gateway_name": gateway_name, "profile_name": profile_name}
        self.verbose_log(f"SD-WAN payload: {json.dumps(payload, indent=2)}")

        try:
            response = self.client.post(
                f"{self.api_url}/api/deployment/join-sdwan-profile",
                json=payload
            )
            response.raise_for_status()
            data = response.json()
            for step in data.get('steps', []):
                self.log(f"    SD-WAN: {step}", "SUCCESS")
            self.log(f"  [OK] {gateway_name} assigned to SD-WAN profile '{profile_name}'", "SUCCESS")
            return True
        except httpx.HTTPStatusError as e:
            detail = e.response.text if e.response is not None else str(e)
            self.log(f"  [WARN] SD-WAN assignment failed for {gateway_name}: {detail}", "WARNING")
            return False
        except Exception as e:
            self.log(f"  [WARN] SD-WAN assignment failed for {gateway_name}: {str(e)}", "WARNING")
            return False
    ...
    def deploy_from_csv(self, csv_path: str, deployment_type: str, dry_run: bool = False,
        ...
        for i, row in enumerate(rows, 1):
            ...
            # SD-WAN profile assignment (Smart-1 Cloud and MDS/SMS only, post-deployment)
            sdwan_enabled = str(row.get('SDWAN', '')).strip().lower() in ('true', '1', 'yes', 'on')
            sdwan_profile = row.get('SDWAN-Profile', '').strip()
            if result.success and sdwan_enabled and sdwan_profile:
                if deployment_type in ('s1c', 'sms'):
                    self.assign_sdwan_profile(result.gateway_name, sdwan_profile)
                else:
                    self.log(
                        f"  [WARN] SDWAN ignored for {result.gateway_name}: "
                        f"SD-WAN is only supported for s1c and sms deployments",
                        "WARNING"
                    )
