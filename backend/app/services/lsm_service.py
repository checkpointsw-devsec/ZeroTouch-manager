"""
Service for managing Check Point LSM (Large Scale Management) operations.
Handles gateway provisioning via Smart Provisioning on Management Servers.
"""
import asyncio
import httpx
...
class LSMService:
    """Service for LSM gateway management operations."""

    def __init__(self):
        """Initialize LSM service with management server configuration."""
        ...
        logger.info(f"Initialized LSM Service")
    ...
    async def add_lsm_gateway(
        ...
        provisioning_state: str = "using-profile"
    ) -> Dict[str, Any]:
        """
        Add LSM gateway to management server.
        ...
        Args:
            ...
            provisioning_state: Provisioning state (default: "using-profile")
        ...
        try:
            logger.info(f"Adding LSM gateway '{name}' to Management Server")

            url = f"{self.base_url}/add-lsm-gateway"
            headers = {
                "Content-Type": "application/json",
                "X-chkp-sid": self.session_id
            }
            ...
            log_http_request(logger.info, "add-lsm-gateway", payload)
            response = await self.client.post(url, headers=headers, json=payload)

            logger.info(f"Response status: {response.status_code}")
            log_http_response(logger.info, "add-lsm-gateway", response.status_code, response.text)

            response.raise_for_status()

            result = response.json()
            logger.info(f"Successfully added LSM gateway '{name}'")

            return result

        except httpx.HTTPError as e:
            logger.error(f"Failed to add LSM gateway: {str(e)}")
            if hasattr(e, 'response') and e.response is not None:
                logger.error(f"Response status: {e.response.status_code}")
                logger.error(f"Response body: {e.response.text}")

                # Check for duplicate gateway name error
                if e.response.status_code == 400:
                    try:
                        error_detail = e.response.json()
                        error_msg = error_detail.get('message', '')
                        if 'More than one object named' in error_msg or 'already exists' in error_msg.lower():
                            raise Exception(
...
async def get_lsm_service() -> LSMService:
    """Dependency injection function for LSM service."""
    return LSMService()
