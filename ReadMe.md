...
|---|---|---|---|
| **Smart-1 Cloud** | Check Point Smart-1 Cloud | Spark + Gaia | Creates gateway in cloud, injects MaaS token, configures blades + VPN |
...
**Optional add-on:** After a successful **Smart-1 Cloud** or **MDS / SMS** deployment, the gateway can be automatically assigned to an **SD-WAN profile** in the Infinity Portal (publish + enforce policy). This is available in both the web UI and the batch CLI.
...
| **Gateway hardware** | Physical gateway appliances with known MAC addresses. a Mac Address list can be extracted over Zero Touch Portal|
| **Network connectivity** | Deployer host (not the admin  host) can reach Zero Touch Portal (HTTPS) and the management server (port 443) |
...
| `SD_WAN_Client_ID` | SD-WAN only | | Infinity Portal OAuth client ID for SD-WAN profile assignment |
| `SD_WAN_Client_SecretKey` | SD-WAN only | | Infinity Portal OAuth secret (access key) for SD-WAN |
| `SD_WAN_Client_Auth_URL` | | `https://cloudinfra-gw.portal.checkpoint.com/auth/external` | Infinity Portal OAuth endpoint |
| `SD_WAN_GRAPHQL_URL` | | `https://cloudinfra-gw.portal.checkpoint.com/app/sd-wan/graphql/v1` | SD-WAN GraphQL API endpoint |
...
| `SDWAN` | | `true`/`false` — assign the gateway to an SD-WAN profile after deployment |
| `SDWAN-Profile` | | SD-WAN profile name (used when `SDWAN` is `true`) |

**Example row:**

```csv
mac_address,account_id,template_id,template_name,gateway_name,sic_otp,hardware,gateway_type,os_version,policy_name,vpn_community,vpn_role,ipv4_address,SDWAN,SDWAN-Profile
00:1C:7F:9B:AE:8D,9075116,570444285,Master_S1C-Spark,gw-1590w-01,vpn123,1570/1590 Appliances,Spark,R81.10,Standard,MucLabLegacy,satellite,192.168.10.130,true,SD-WAN Gateways
```
...
```csv
mac_address,account_id,template_id,template_name,gateway_name,mgmt_server_ip,sic_otp,gateway_ipv4,version,hardware,policy_name,enable_ipsec,vpn_community,vpn_role,domain,SDWAN,SDWAN-Profile
00:1C:7F:CC:E0:8A,9075116,754093852,Master_MDS-3950,gw-3970-01,192.168.10.78,vpnChangeMe,192.168.10.86,R82.10,3900 Appliances,Standard,false,MucLabTest,satellite,Sub_Dom1,true,SD-WAN Gateways
```
...
- **SD-WAN assignment** — the `SDWAN` / `SDWAN-Profile` columns are honored only for the **s1c** and **sms** deployment types. They are ignored for LSM and SMP. The `SD_WAN_*` credentials must be set in `.env`.
...
**SD-WAN profile (optional):** When the management platform is **Smart-1 Cloud** or **MDS / SMS**, an **"Assign to SD-WAN profile"** checkbox appears below the platform selector. Tick it and enter the SD-WAN profile name to have the gateway added to that profile in the Infinity Portal automatically after the deployment completes (the assignment is published and the SD-WAN policy enforced). The checkbox is hidden for LSM and SMP. SD-WAN credentials (`SD_WAN_*`) must be configured in `.env`.
...
**SD-WAN assignment:** If the SD-WAN checkbox was enabled (Smart-1 Cloud / MDS / SMS only), the SD-WAN assignment steps are appended to the deployment log after the gateway deployment succeeds. An SD-WAN failure is shown as a warning and does **not** fail the gateway deployment itself.
...
| POST | `/api/deployment/join-sdwan-profile` | Assign a deployed gateway to an SD-WAN profile (Smart-1 Cloud / SMS) |
...
├── join-sdwan-profile.py        # Standalone CLI — assign a gateway to an SD-WAN profile
...
│   │   └── services/            # Orchestrators and service clients (incl. sdwan_service.py)
